...
**本账本最后记录的 staging schema head = 0068（2026-09-14 文档对账：应用证据见下方 2026-08-23 条目；即时线上 schema 仍须现场核实）。** 0063–0067 的应用记录见各日期条目，0065 晚于 0066/0067 应用。缺号与 open PR 的现状须在分配新号前重新核实，编号不代表连续应用。
(此行 2026-08-02 修正:此前长期停在 0052,0054/0055 只记在变更历史漏更此行。)

**0070 待应用（2026-09-14）**：backend #276 尚未合并；原始 SQL 已通过隔离 PostgreSQL 17 的空表、存量回填与 nextval 验证，见 [验收记录](../docs/verification-0070-pg17-2026-09-14.md)。线上迁移、新镜像 rollout、`COACH_PLAN_SHIFT_ENABLED` 启用和 web #101 换装均未执行；以下已应用/部署历史不代表本批完成。
