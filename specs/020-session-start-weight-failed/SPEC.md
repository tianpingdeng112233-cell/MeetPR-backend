...
- **派生信号** `weight_failed`（severity **yellow**，status open）：同学员同教练**同 gym-day 合一条**——当日已有 open 且 payload.gym_day 相同 → 更新（最新一组入 reason + `failed_count`++），否则走 LOCK CONTRACT（users 行锁）select-then-update/insert。reason 人话如「卧推 120kg 未完成」/合条后「卧推 120kg 未完成（今日第 2 次）」（组号不引用 set_index——历史基底混杂不可稳定推导，failed_count 是自有可靠计数；review-loop 轮 1 修订）。跨日残留 open 行原位翻新为今日（open 部分唯一索引一对一行，插新必 23505）。`expires_at` = opened_at + `SIGNAL_POLICY.signalExpiryDays`。
