...
### 1. migration `0029-init-events.sql`

> **编号（实装 2026-07-04 重编）**：spec 起草时写 0019，但 0019–0028 已被后续 catalog/plan wave 占用（staging 头 = 0028）。实装取号 = staging 实际最大号 +1 → events 用 **0029**、analytics_feedback 用 **0030**。下方字面 SQL 注释同步改号。

仓库**无 migration runner**（手 psql 应用，见 CLAUDE.md Hard rule 2）。文件落 `db/migrations/0029-init-events.sql`，BEGIN/COMMIT 包裹，镜像 0014/0015 文件头注释风格。**字面 SQL**：
...
**migration `0030-init-analytics-feedback.sql`**（紧随 0029）：
...
**`tests/migrations/0029-events.test.ts` + `tests/migrations/0030-analytics-feedback.test.ts`（pg-mem）：**

- 0029：`event_id` UNIQUE 拒重复插入；`user_id` FK `ON DELETE SET NULL`（删 user 后该行 `user_id IS NULL`、行**仍在**，对比 0014 的 CASCADE）；`props` 默认 `'{}'`、`platform` 默认 `'ios'`、`schema_version` 默认 1；四索引存在。
- 0030：`analytics_feedback` 建表；`event_id` UNIQUE；`user_id` FK `ON DELETE SET NULL`；`text` NOT NULL；两索引存在。
...
5. **部署顺序**（仓库无 migration runner，手 psql）：
   1. `0029-init-events.sql` + `0030-init-analytics-feedback.sql` 合 staging → CI 出 image
...
| 2026-07-04 | 0.5  | 实装取号：0019→**0029**、0020→**0030**（0019–0028 已被 catalog/plan wave 占用，staging 头 0028）。仅编号重编，表结构/端点/CRITICAL 不变。                                                                                                                                                                                                              | Claude |
