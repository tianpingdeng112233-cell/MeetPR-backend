...
>
> **不做整盘 `POST /plans/:id/replace`。** `set_logs.plan_exercise_id` 是 `ON DELETE CASCADE`（`db/migrations/0005-init-set-logs.sql`），`feedback.plan_exercise_id` 是 `ON DELETE SET NULL`（`0006-init-feedback.sql`）。删一个 `plan_exercise` 会**连带删掉学员在该动作上的全部 `set_logs`、清空教练反馈关联**。现有 `reconcile.ts` 之所以「只重建变化的天、保住没变的天的 id」，正是为了保住学员训练记录。一个天真的整盘删重建会把已发布、学员正在练的计划的全部训练记录清光 = 数据丢失。批量端点必须**逐字复刻天级差量语义**。
>
...
4. **一个事务**（`deps.db.transaction().execute(async (trx) => {…})`，沿用 `/:id/publish` 已有事务范式）：
   ...
   2. **删**：`delete_day_ids` 非空 → `deleteFrom('plan_days').where('plan_id','=',planId).where('id','in',delete_day_ids)`。`plan_id` 兜底 = 客户端传别的计划的 day id **删不动**（静默 no-op，不 trust client id）。级联删 exercises/sets/set_logs —— 与今日逐天 `deleteDay` 完全一致。
...
- **`reconcilePlan(planId, weeks)`**：遍历 (week, dow)，对每个**变化的天**：
...
- **整盘 `POST /plans/:id/replace`**：cascade 删学员 `set_logs` = 数据丢失。天级差量是硬约束。
- **天内 exercise/set 级 in-place 保 id**：现语义是天级替换（改一天即删重建该天全部 exercises），批量忠实复刻、不改进。import 是草稿无学员记录，无收益。YAGNI。
