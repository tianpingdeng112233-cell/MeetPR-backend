...
- **拍板记录**:
  ...
  - 2026-08-09(v2)David 扩板,参照竞品 PowerSheets 强度体系:
    ...
    4. % 的锚点问题先不展开:v1 按「1RM 的百分比」理解,不引入 TM 概念(pct_of_tm
       留给算法引擎,见 §3.3)。
  - 2026-08-09(v2.1)David 拍板甲「值层逐组化」:强度**类型仍行级**,但单值类型的
    **值可逐组**(交互同逐组标重);借此消灭「旧逐组 RPE」兼容伪类型——存量逐组 RPE
    即「rpe 类型 + 逐组值」一等公民。两条硬约束:**重量列只显示 kg**;
    **逐组 RPE 等一切强度值只住强度列**。
  - 2026-08-12(v2.2)David 拍板 B「% 锚点二级化」:% 仍是单一强度类型,选中后
    二级选锚 **1RM / e1RM / 当日顶组**,默认 1RM(存量零回填);顶组判定的
    「前序行规则」同日确认。详见 §9。
...
- **强度列**(类型行级六选一;单值类型的**值可逐组**,v2.1):
  | 类型 | 值形态 | 例 |
  | ----------------------- | --------------------------------------------------------- | ------------------------ |
  | `pct` % | 单值,20.0–110.0,0.5 步进;**可逐组**;锚点二级选择(v2.2,§9) | 72.5% 或 70/72.5/75/77.5 |
  | `rpe` | 单值,1.0–10.0,0.5 步进;**可逐组** | RPE 8 或 6/7/8/8 |
  | `rir` | 单值整数,0–9;**可逐组** | RIR 2 |
  | `weight_range` 重量区间 | 两值 kg,low < high;仅行级(逐组区间 v1 不做) | 165–175kg |
  | `rpe_range` RPE 区间 | 两值,1.0–10.0,0.5 步进,low < high;仅行级 | RPE 7–8 |
  | `fixed_weight` 固定重量 | **无独立值**,具体数字填重量列 | 「固定重量」+ 重量列 170 |
  - 值输入默认一格 = 各组同值;单值类型可切「逐组」= 每组一框(交互同逐组标重),
    允许稀疏留空(top single 写法:只给第 1 组值或只给 back-off 组值)。
  - **强度值只住强度列**(含逐组 RPE);存量逐组 RPE 数据 = `rpe` 类型 + 逐组值,
    一等公民渲染,**不设「旧逐组 RPE」伪类型**。
- **重量列**(选填,**只显示 kg**):默认一格 = 该行各组同重;切「逐组标重」= 每组一框,
  逐组可不同(现状模式,原样保留)。
- **完成判定**(按组):该组强度值或该组重量任一有值即完成;两者同填 = 双锚(如 RPE7 + 170kg)。
- **组合约束**:`fixed_weight` 必须带重量列;`weight_range` 不得再带重量列(区间与
  定值矛盾,422);其余四种类型与重量列自由组合。
- RIR 不换算成 RPE 存储,原样落库。落库语义随 v2.1 更新:强度值按组落
  (行级一格 = 各组同值的便捷输入姿态,非模型约束);类型行级 = 同行各组 `load_mode` 一致。
...
spec 里做。**本波不引入 TM 概念**(David 2026-08-09:% 锚点先不展开;v2.2 的
锚点二级化落在 `pct_anchor`,依旧不引 TM,见 §9)。
...
## 9. v2.2 增补:% 锚点二级化(2026-08-12 拍板 B)

### 9.1 产品口径

强度类型下拉**不膨胀**:`pct` 仍是一项「%」。选中 % 后出现二级锚点选择,三选一,
**默认 1RM**:

| 锚点               | 白话语义                                      | 换算时机                |
| ------------------ | --------------------------------------------- | ----------------------- |
| `one_rm` 1RM       | 学员登记的实测 1RM 的百分比                   | 静态,写计划时即可换算   |
| `e1rm` e1RM        | 学员当前 e1RM(头条口径)的百分比               | 动态,随打卡自动更新     |
| `top_set` 当日顶组 | 当日同动作顶组实际重量的百分比(back-off 写法) | 运行时,顶组打完才可换算 |

- 锚点是 `pct` 类型的**行级**修饰(随类型走,不逐组);% 值本身照旧可逐组、可稀疏。
- 值域/步进不变(20.0–110.0,0.5 步进)。
- 锚点标签随 % 值一起展示(如「88% ×顶组」);具体样式归 W5 卡。

### 9.2 数据模型

(迁移编号实装时现场取号,勿照字面建;写作本节时迁移目录 head = 0062。)
...
- **NULL 语义 = `one_rm`**:存量 `pct` 行与不带锚的新写入都按「1RM 的百分比」理解,
  与 v2 白话语义一致,**零回填**。
- `load_mode ≠ 'pct'` 时 `pct_anchor` 必须 NULL;换出 pct 模式时服务端替调用方清
  (§4「不 422 刁难」原则),DB CHECK 兜底。
- 与 0033 算法列的分工延续 §3.3:`pct_anchor` 只修饰教练手写 `target_pct`;
  `pct_of_tm` / `method_anchor` / `backoff_pct` 仍归算法引擎,**不复用**——尤其
  `backoff_pct` 语义近 `top_set` 但属算法处方位,混用会把 TM/引擎校验逻辑拖进
  教练手写路径。

### 9.3 API

- `PlanSetBodySchema` / patch / batch 增 `pct_anchor`(三值枚举,可空):
  非空且 `load_mode ≠ 'pct'` → 422(直传矛盾);patch 合并态换出 pct → 服务端清锚。
  写 `pct` 不带锚 → 存 NULL(= 1RM)。
- 全部带 sets 的响应出口带出 `pct_anchor`。
- **老客户端投影不变**:`pct` 无重量照走 §4 分段映射表,三种锚同表。`top_set` 锚下
  该映射语义上更粗(88% ×顶组 ≈ 高强度但非 1RM 的 88%),接受——粗投影只求老端
  不崩且方向正确,新客户端不读它。

### 9.4 学员端换算语义(iOS 六形式渲染波的实装依据)

总则(2026-08-12 学员端总则的延伸):六形式是教练语言,学员端负责翻译成**建议重量**;
**解析不出锚值 → 只显示 % 原文,不硬给建议**;建议重量取整沿用现有建议引擎口径,
不新立规则。

- **`one_rm`**:锚 = onboarding 档案登记 1RM(`student_onboarding_profiles.
...
- **`top_set` 当日顶组,五条钉死**:
  1. **候选域 = 前序行**:同一训练日(同 `plan_day`)内、同 `exercise_id`、且
     ...
     自身的组会进全天集合,学员某组打重会把后续建议顶着往上爬(棘轮回路);
     前序行规则下顶组行写在 back-off 行之后 = 候选域空 = 解析失败,行为可预期。
     adhoc 打卡不属于任何行,天然不计入。
  2. **锚 = 候选域内已打卡组的最大实际重量**:计入门槛为 `completed` 且非
     `failed`、`weight_kg > 0`、`reps ≥ 1`(试举失败不算顶组)。锚是那个重量数字,
     不是某组的身份,同重多组无所谓。
  3. **只在执行态解析实际值**:web 编辑器与学员未开练时只显示语义(「当日顶组
     ×88%」),不做计划态预估——省掉「前序行是纯 RPE/区间怎么预估」整层边界。
  4. **锚是活的,但不回溯**:前序行补打/修改实际重量 → 未打卡组的建议重量重新
     解析;**已打卡组记录的是实际值,永不回改**。学员乱序先打 back-off(只见 %、
     无建议)是本条自然结果,属已知行为。
  5. 全部由「实际重量/次数/RPE」三件套打卡推导,**零新输入**,不给学员新增任何
     输入形式。

### 9.5 分工与验收

- **W4 backend(本仓)**:`pct_anchor` 一列迁移 + schema 校验 + 序列化 + 测试;
  合并后部署 staging 并记账。验收:
  1. 三值锚随 `load_mode='pct'` 写入回读全对(create / patch / batch);
  2. 非 pct 直传锚 → 422;patch 换出 pct → 锚被清;写 pct 不带锚 → NULL;
  3. 存量行与不带新字段的旧请求零感知(回归);投影字段 shape 与取值不变(快照);
  4. 迁移在干净库与含存量库均可应用。
- **W5 plan-web(另卡,依赖 W4 部署)**:% 选中后二级锚点选择器(默认 1RM),
  保存/回读/发布门禁照旧(% 值门禁不变)。
- **iOS(并入学员端六形式渲染波,卡 2)**:按 §9.4 换算;不阻塞 1.0(19) 的
  止血卡 1。
