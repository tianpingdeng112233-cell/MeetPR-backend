...
2. 按下节的隔离恢复方案验证最新持久托管备份；完成恢复与候选迁移演练并记录汇总断言，才允许生产迁移。凭证与用户数据不写日志/PR/本机 scratch。
...
使用现有 DigitalOcean 账号，在源库相同 region/VPC 中，从审批执行时可用的最新事务恢复成**新建临时 PostgreSQL 17 集群** `meetpr-global-w3-restore-20260923`；保留原生产集群及绑定，不把 App 接到副本。DigitalOcean 恢复会创建新的主集群，支持最近事务或时间点恢复，日备份保留七天。[官方恢复说明](https://docs.digitalocean.com/products/databases/postgresql/how-to/restore-from-backups/)

审批范围建议为：创建该临时集群、仅访问该副本进行恢复校验和0070演练、验证后删除**本次创建且ID已记录的临时副本**。拟定费用上限 **US$5、存续不超过24小时**；这是申请上限，不是报价。创建前只读核实源 region/VPC、规格、账号配额与实际预估费用；采用可恢复且落在上限内的最小单主规格。若费用/规格不能明确核实、超预算、配额不足或私密访问无法建立，停止，不删其他集群腾配额。官方当前标准 PostgreSQL 单主1GiB起价为 $0.02254/小时，最终以创建页/API报价及存储费用为准。[官方价格](https://www.digitalocean.com/pricing/managed-databases)

访问从临时 GitHub Actions runner 通过 TLS `verify-full` 完成；只在副本配置该 runner 的 /32 trusted source，不开放0.0.0.0/0、不修改生产防火墙。凭证使用既有获准secret通道，恢复数据仅留DigitalOcean副本，不导出本机或上传artifact。日志只保留行数/账本/约束断言。核对恢复后的账本0068、计划/日志/视频等汇总数量，再在副本对固定候选原子应用0070及ledger，验证批次回填、seq非空与原数量不减少。恢复点后的生产新写入可能造成计数差异，按恢复时间解释，不能把不同时刻计数强行认作一致快照。生产迁移前再次验证备份新鲜度及pending集合。

本节审批同时覆盖上述临时副本清理；不包含删除或覆盖任何现有生产资源，也不包含将备份覆盖回生产。生产0070、默认关门部署、兼容检查后启用教练改期，仍按上一节顺序和停止条件执行。
